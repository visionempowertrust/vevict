...
insert into ct_outcomes (
  ...
  ('CT-EVA', 'Evaluation', 'Makes moves without checking against the goal or comparing options.', 'Checks the goal or compares options with prompts.', 'Independently checks outcomes, compares strategies, and selects a suitable move.', 'Explains why one strategy is better and considers efficiency/fairness/accessibility.'),
...
insert into ct_suboutcomes (suboutcome_code, outcome_code, suboutcome_name, suboutcome_description) values
  ...
  ('CT-LOG-SO2', 'CT-LOG', 'Draws an inference', 'Reaches a reasonable conclusion from available evidence'),
  ('CT-LOG-SO3', 'CT-LOG', 'Uses cause and effect', 'Explains how an action produces a game consequence'),
  ('CT-LOG-SO4', 'CT-LOG', 'Justifies a choice', 'Provides a logical reason for selecting one move or strategy'),
  ('CT-DAT-SO1', 'CT-DAT', 'Collects data accurately', 'Records game events, counts, scores or observations consistently'),
  ('CT-DAT-SO2', 'CT-DAT', 'Organises data', 'Groups or orders information so it can be used'),
  ('CT-DAT-SO3', 'CT-DAT', 'Represents data', 'Uses a tally, table, chart, tactile display or verbal summary'),
  ('CT-DAT-SO4', 'CT-DAT', 'Interprets data', 'Uses recorded information to describe trends or make decisions'),
  ('CT-EVA-SO1', 'CT-EVA', 'Checks against the goal', 'Compares the result with the game objective or success condition'),
  ('CT-EVA-SO2', 'CT-EVA', 'Compares strategies', 'Identifies strengths and limitations of two approaches'),
  ('CT-EVA-SO3', 'CT-EVA', 'Considers efficiency', 'Notices time, number of moves, effort or materials used'),
  ('CT-EVA-SO4', 'CT-EVA', 'Considers accessibility and fairness', 'Notices whether rules, materials and participation work for everyone'),
...
from (values
  ...
  ('CONNECT FOUR', 'CT-EVA', 'Child checks possible lines of four, compares own and opponent opportunities, and selects efficient moves to win or block.'),
  ...
  ('LAST MAN STANDING', 'CT-EVA', 'Child checks number formation against win condition, compares arrangements, and chooses efficient place-value strategy.'),
  ...
  ('NOUGHTS & CROSSES', 'CT-EVA', 'Child checks moves against the goal of making a line, compares available moves, and chooses an efficient blocking/winning move.'),
  ...
  ('RACE TO 27', 'CT-EVA', 'Child checks running total against 27, compares possible cards/moves, and chooses efficient moves toward the target.'),
