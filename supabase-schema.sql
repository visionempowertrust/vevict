...
create table if not exists registered_students (
  ...
  other_physical_disabilities text check (other_physical_disabilities is null or other_physical_disabilities in ('Yes', 'No')),
  cognitive_disabilities text check (cognitive_disabilities is null or cognitive_disabilities in ('Yes', 'No')),
  is_braille_literate text check (is_braille_literate is null or is_braille_literate in ('Yes', 'No')),
  braille_reading_level text check (braille_reading_level is null or braille_reading_level in ('Letters', 'Words', 'Sentences')),
  braille_writing_level text check (braille_writing_level is null or braille_writing_level in ('Letters', 'Words', 'Sentences')),
  knows_taylor_frame text check (knows_taylor_frame is null or knows_taylor_frame in ('Yes', 'No')),
  knows_nemeth text check (knows_nemeth is null or knows_nemeth in ('Yes', 'No')),
  knows_using_computer text check (knows_using_computer is null or knows_using_computer in ('Yes', 'No')),
  knows_maths_on_computer text check (knows_maths_on_computer is null or knows_maths_on_computer in ('Yes', 'No')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
